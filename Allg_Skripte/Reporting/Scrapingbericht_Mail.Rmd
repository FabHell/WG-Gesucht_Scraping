...
alle_staedte <- c("Wien", "München", "Berlin", "Hannover", "Kassel", "Hamburg", 
                  "Leipzig", "Bremen", "Kiel", "Erfurt", "Rostock", "Zürich")
...
Wochenreferenz <- Daten_ges %>%
  ...
  pivot_wider(names_from = datum_scraping, values_from = Anzahl,
              values_fill = 0) %>%
  mutate(veraenderung = ifelse(vor_7_tagen == 0, NA, (heute - vor_7_tagen) / vor_7_tagen * 100))
...
logs_heute <- alle_staedte %>%
  map(function(stadt) {
    list.files(
      ...
      pattern = paste0("Log_", stadt, " ", format(Sys.Date(), "%d.%m.%Y"), " \\{\\d+\\}\\.txt"),
      full.names = TRUE
    )
  }) %>%
  unlist()
...
```{r abbildung, include=TRUE, fig.height=7, fig.width=4.5}
...
Abb1 <- log_daten_gesamt %>%
  ...
  scale_fill_manual(values = c("0" = "#84b884", "1" = "#f1a765",
                               "2" = "#d97c7c", "3" = "#cfcfcf")) +
  facet_wrap(~ Stadt, ncol = 1, strip.position = "left") +
...
Abb2 <- ggplot(Wochenreferenz, aes(y = stadt, x = veraenderung)) +
  geom_linerange(aes(xmin = 0, xmax = veraenderung, color = veraenderung > 0),
                 linewidth = 7.5) +
  geom_richtext(aes(x = 140, label = paste0(vor_7_tagen, " / ", heute)), 
                size = 3, color = "gray20", fill = "#f8f5f2", fontface = "bold") +
