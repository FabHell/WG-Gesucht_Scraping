...
```{r abbildung, include=TRUE, fig.height=13.5, fig.width=5}


log_städte <- log_daten_gesamt %>% 
  group_by(Stadt, Scraping_Status) %>%
  summarise(count = n(), .groups = "drop") 

log_gesamt <- log_städte %>% 
  group_by(Scraping_Status) %>%
  summarise(count = sum(count)) %>%
  mutate(Stadt = "GESAMT", .before = "Scraping_Status")

Abb1 <- rbind(log_gesamt, log_städte) %>%
  mutate(fill = case_when(
    Stadt != "GESAMT" & Scraping_Status == 0 ~ "#84b884",
    Stadt != "GESAMT" & Scraping_Status == 1 ~ "#f1a765",
    Stadt != "GESAMT" & Scraping_Status == 2 ~ "#d97c7c",
    Stadt != "GESAMT" & Scraping_Status == 3 ~ "#cfcfcf",
    Stadt == "GESAMT" & Scraping_Status == 0 ~ "#4a9d4a",
    Stadt == "GESAMT" & Scraping_Status == 1 ~ "#e8873d",
    Stadt == "GESAMT" & Scraping_Status == 2 ~ "#c94545",
    Stadt == "GESAMT" & Scraping_Status == 3 ~ "#9a9a9a")) %>%
  mutate(Scraping_Status = factor(Scraping_Status),
         Stadt = factor(Stadt, levels = unique(Stadt))) %>%
  
  ggplot(aes(x = Scraping_Status, y = 1, fill = fill)) +
  ...
  theme(
    ...
    plot.margin = margin(t=0,b=0,r=-5,l=0),
    strip.text.y.left = element_text(size = 8, angle = 0, 
                                     face = "bold", hjust = 1)
  )


Wochenreferenz_städte <- Daten_ges %>%
...
Wochenreferenz_GESAMT <- Wochenreferenz_städte %>%
  summarise(vor_7_tagen = sum(vor_7_tagen),
            heute = sum(heute)) %>%
  mutate(stadt = "GESAMT") %>%
  mutate(veraenderung = (heute - vor_7_tagen) / vor_7_tagen * 100) %>%
  mutate(veraenderung_aufb = ifelse(veraenderung > 100, 100, veraenderung),
...
plot_data <- rbind(Wochenreferenz_GESAMT, Wochenreferenz_städte) %>%
  mutate(stadt = factor(stadt, levels = unique(stadt))) %>%
  mutate(color =  case_when(
    stadt != "GESAMT" & veraenderung_aufb > 0 ~ "#5CA4C4",
    stadt != "GESAMT" & veraenderung_aufb < 0 ~ "#E58E89",
    stadt == "GESAMT" & veraenderung_aufb > 0 ~ "#3E8CAF",
    stadt == "GESAMT" & veraenderung_aufb < 0 ~ "#D96860"
  )) %>%
  mutate(fill = ifelse(stadt == "GESAMT", "#DCCFBB","#F6F3EE"))

Abb2 <- plot_data %>%
  ggplot(aes(y = stadt, x = veraenderung_aufb)) +
  geom_linerange(aes(xmin = 0, xmax = veraenderung_aufb, 
                     color = color), linewidth = 6) +
  geom_segment(data = subset(plot_data, veraenderung >= 100),
               aes(x = 80, xend = 100, y = stadt, yend = stadt),
               color = "#3E6B89", linewidth = 6) +
  geom_text(data = subset(plot_data, veraenderung >= 100),
            aes(x = 90, y = stadt, label = "+"),
            color = "#5CA4C4", size = 4, fontface = "bold") +
  geom_richtext(aes(x = 140, label = paste0(vor_7_tagen, " / ", heute), fill = fill),
                size = 3, color = "gray20", fontface = "bold", label.padding = unit(0.1, "lines")) +
  geom_vline(xintercept = 0, linetype = "dashed", color = "gray40") +
  facet_wrap(~stadt, ncol = 1, scales = "free_y", drop = FALSE) +
  scale_x_continuous(limits = c(-110, 165),
...
Abb1 + Abb2 +
  plot_layout(nrow = 1, widths = c(0.35, 1)) &
  theme(plot.margin = margin(0, 0, 0, 0, "pt"),
        panel.spacing = unit(c(6,rep(1.5, nrow(plot_data)-2)), "pt"))
