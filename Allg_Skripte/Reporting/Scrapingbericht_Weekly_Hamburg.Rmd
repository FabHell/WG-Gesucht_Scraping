...
-  Vollständig erfolgreiche Scrapingloops traten um [**`r log_daten_gesamt_hamburg %>% group_by(Uhrzeit, Scraping_Status) %>% summarise(count = n(), .groups = "drop") %>% filter(Scraping_Status == 0) %>% slice_max(count, n = 1) %>% pull(Uhrzeit)`**]{style="color: #84b884;"} Uhr am häufigsten auf. Um [**`r log_daten_gesamt_hamburg %>% group_by(Uhrzeit, Scraping_Status) %>% summarise(count = n(), .groups = "drop") %>% filter(Scraping_Status == 2) %>% slice_max(count, n = 1) %>% pull(Uhrzeit)`**]{style="color: #d97c7c;"}<br>Uhr scheiteren trotz neuer Links die meisten Scrapingloops.
-  Von den Scrapingzyklen mit neuen Links waren [**`r nrow(log_daten_gesamt_hamburg %>% filter(Scraping_Status == 0))`**]{style="color: #84b884;"} Loops vollständig erfolgreich, [**`r nrow(log_daten_gesamt_hamburg %>% filter(Scraping_Status == 1))`**]{style="color: #f1a765;"} teilweise<br>erfolgreich und bei [**`r nrow(log_daten_gesamt_hamburg %>% filter(Scraping_Status == 2))`**]{style="color: #d97c7c;"} der Loops keine Datenabfragen möglich.
