...
log_pfade <- c(
  ...
  "C:/Users/Fabian Hellmold/Desktop/WG-Gesucht-Scraper/Zürich/Logs",
  "C:/Users/Fabian Hellmold/Desktop/WG-Gesucht-Scraper/Hannover/Logs"
  )

logs_heute <- map(log_pfade, function(pfad) {
  list.files(
    path = pfad,
...
-   [**`r nrow(log_daten_gesamt %>% filter(Scraping_Status == 0))`**]{style="color: #84b884;"} der Loops vollständig erfolgreich.
-   [**`r nrow(log_daten_gesamt %>% filter(Scraping_Status == 1))`**]{style="color: #f1a765;"} nur teilweise erfolgreich.
-   bei [**`r nrow(log_daten_gesamt %>% filter(Scraping_Status == 2))`**]{style="color: #d97c7c;"} der Loops keine Datenabfragen möglich.
...
log_daten_gesamt %>% 
  mutate(Stadt = factor(Stadt, levels = c("Zürich",  "Wien", "Berlin", "München", 
...
ggplot(aes(x = Seite, y = 1, fill = as.factor(Scraping_Status))) +  
  ...
  facet_grid(Stadt ~ Uhrzeit, switch = "both", scales = "free_y") +
  labs(title = NULL, x = NULL, y = NULL) +
  theme_minimal() +
  theme(
...
```{r echo=FALSE, include = FALSE, results='asis', fig.height=3.5, fig.width= 3.75}
